...
export const EVALUATOR_DESCRIPTIONS: Record<EvaluatorName, string> = {
  faithfulness:
    'Quando o agente afirma um preço, prazo ou característica, ele está se baseando em informação real (base de conhecimento) ou inventou? Quanto maior, mais confiável.',
  goalAccuracy:
    'O agente está conduzindo a conversa em direção ao objetivo dele (qualificar, agendar, responder dúvida, encaminhar)? Quanto maior, mais focado.',
  toolUsage:
    'O agente usou as ferramentas certas no momento certo (buscar na base, criar lead, transferir, etc.)? Penaliza quando o agente "esquece" de buscar antes de responder.',
  scopeAdherence:
    'O agente respeitou as regras e o escopo definidos no prompt? Penaliza desvios para assuntos fora do contexto que ele foi treinado pra atender.',
  humanization:
    'A resposta soa natural, como uma pessoa escreveria? Penaliza narração interna ("vou buscar...", "deixa eu pensar...") ou texto técnico/robótico.',
  actionCompleteness:
    'Em cenários de teste com ações esperadas, o agente chamou todas as ferramentas que se esperava? Mede execução completa, não só intenção.',
  latencyBudget:
    'Mede o tempo total de resposta comparado ao tempo aceitável (SLA padrão: 10s). 100% quando responde abaixo do SLA, cai linearmente até 0% quando demora o dobro. A latência inclui o tempo do modelo de IA + chamadas de ferramentas (busca, RAG, etc.).',
  transparency:
 ...
 */
const EVALUATOR_ALIASES: Record<string, EvaluatorName> = {
 ...
 */
export function normalizeEvaluatorName(name: string): EvaluatorName | null {
  if (name in EVALUATOR_LABELS) return name as EvaluatorName;
 ...
 */
export function getEvaluatorLabel(name: string): string {
  const canonical = normalizeEvaluatorName(name);
  if (canonical) return EVALUATOR_LABELS[canonical];
  return name
    .replace(/[_-]+/g, ' ')
...
export function getEvaluatorDescription(name: string): string {
  const canonical = normalizeEvaluatorName(name);
  if (!canonical) return '';
  return EVALUATOR_DESCRIPTIONS[canonical];
}
