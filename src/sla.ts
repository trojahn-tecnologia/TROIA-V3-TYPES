 ...
 */
export const SLA_CLOCK_KEYS: readonly SlaClockKey[] = ['first_response', 'resolution'];
...
export const SLA_DEFAULT_POLICY_ORDER = 999;
...
export interface SlaEscalationRow {
  ...
  notify: { assignee: boolean; managers: boolean; teamIds: string[] };
}
...
export type SlaEventType =
  | 'policy_applied' | 'start' | 'pause' | 'resume' | 'target_changed'
  | 'breach' | 'meet' | 'cancel' | 'discard';
...
export interface SlaClockEvent {
  ...
  actor: { type: 'system' | 'user' | 'workflow' | 'agent'; id?: string };
  snapshot?: { policyId: string; policyVersion: number; targetMs: number; useBusinessHours: boolean };
  elapsedBusinessMs: number;
  elapsedCalendarMs: number;
}
