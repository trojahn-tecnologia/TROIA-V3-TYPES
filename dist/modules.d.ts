 ...
 */
export type ValidModuleId = 'app' | 'users' | 'companies' | 'levels' | 'plans' | 'channels' | 'integrations' | 'company-integrations' | 'payments' | 'crm' | 'chat' | 'leads' | 'customers' | 'activities' | 'conversations' | 'conversation-messages' | 'tickets' | 'assignments' | 'contacts' | 'groups' | 'group-participants' | 'funnels' | 'funnel-steps' | 'goals' | 'products' | 'orders' | 'campaigns' | 'audiences' | 'templates' | 'quick-messages' | 'calendar' | 'calls' | 'reports' | 'knowledge' | 'company' | 'teams' | 'skills' | 'shifts' | 'services' | 'email-templates' | 'email-domains' | 'saved-cards' | 'api-keys' | 'agents' | 'tools' | 'databases' | 'workflows' | 'websites' | 'voices' | 'forms' | 'ticket-pipelines' | 'ticket-stages' | 'dashboards-commercial' | 'dashboards-support' | 'dashboards-marketing' | 'quality';
